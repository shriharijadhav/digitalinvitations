...
import { saveHaldiTime } from '../../store/actions';

const TimePickerForHaldi = ({onClose}) => {
    ...
    const handleSelect = () => {
        ...
        dispatch(saveHaldiTime(selectedTime))
        onClose()
    ...
    return (
             <Flex direction={'column'} gap={'40px'}  w={['100%','100%','50%','30%']}  borderWidth="1px" borderRadius="lg" overflow="hidden" p={5} boxShadow="md" mt={5}>
                <Flex w={'100%'} direction={'column'} spacing={4}>
                    <Text p={'10px 0px'} fontSize={['large','large','x-large','x-large']} fontWeight={['500','500','700','700']}> Select Haldi Time
                    </Text>
...
export default TimePickerForHaldi;
