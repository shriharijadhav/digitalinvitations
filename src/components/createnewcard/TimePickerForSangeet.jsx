...
import { saveSangeetTime } from '../../store/actions';

const TimePickerForSangeet = ({onClose}) => {
    ...
    const handleSelect = () => {
        ...
        dispatch(saveSangeetTime(selectedTime))
        onClose()
    ...
    return (
             <Flex direction={'column'} gap={'40px'}  w={['100%','100%','50%','30%']}  borderWidth="1px" borderRadius="lg" overflow="hidden" p={5} boxShadow="md" mt={5}>
                <Flex w={'100%'} direction={'column'} spacing={4}>
                    <Text p={'10px 0px'} fontSize={['large','large','x-large','x-large']} fontWeight={['500','500','700','700']}> Select Sangeet Time
                    </Text>
...
export default TimePickerForSangeet;
