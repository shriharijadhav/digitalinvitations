...
import { makeEngagementAddressSameAsWedding, previousPage, proceedToNextPage, saveEngagementAddress, saveEngagementDate, saveEngagementTime, toggleAddEngagementDetails, toggleAddInviterDetails } from '../../store/actions'
import {
    Modal,
    ModalOverlay,
    ModalContent,
...
const C_Inviter = () => {
  ...
  return (
        ...
        addInviterDetails && (
            <Flex border={'1px solid grey'} w={'100%'} gap={'10px'} direction={'column'} p={'10px 10px'} borderRadius={'10px'}>
                <Flex w={'100%'} direction={['column','column','row','row']} gap={'20px'} justifyContent={'left'} alignItems={['start','start','center','center']}>
                    ...
                    <Input placeholder="Enter inviter's first name" />            
                </Flex>
                <Flex w={'100%'} direction={['column','column','row','row']} gap={'20px'} justifyContent={'left'} alignItems={['start','start','center','center']}>
                    <Text w={['100%','100%','45%','45%']} pr={'20px'}>Last Name :</Text>
                    <Input placeholder="Enter inviter's last name" />            
                </Flex>
                <Flex w={'100%'} direction={['column','column','row','row']} gap={'20px'} justifyContent={'left'} alignItems={['start','start','center','center']}>
                    <Text w={['100%','100%','45%','45%']} pr={'20px'}>Email:</Text>
                    <Input placeholder="Enter inviter's email" />            
                </Flex>
                <Flex w={'100%'} direction={['column','column','row','row']} gap={'20px'} justifyContent={'left'} alignItems={['start','start','center','center']}>
                    <Text w={['100%','100%','45%','45%']} pr={'20px'}>Contact Number :</Text>
                    <Input placeholder="Enter inviter's contact number" />            
                </Flex>
            </Flex>
        )
    }
    ...
    {/*<Modal size={['lg']}  isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent m={'auto'} p={'30px 0px'} >
           <ModalCloseButton />
          <ModalBody>
            <Text fontSize={'large'}>Please confirm if you wish to submit your details.</Text>
          </ModalBody>

          <ModalFooter w={'100%'} display={'flex'} justifyContent={'center'}>
            <Button colorScheme='purple' mr={3} onClick={onClose} isDisabled={false}>
              No
            </Button>
            <Button
            isLoading={false}
    ...
    <Flex  w={'100%'} gap={'10px'} justifyContent={'center'}  borderRadius={'10px'}>
        <Button w={'80%'} colorScheme={'purple'}  leftIcon={<FaEye />}>
            Preview
        </Button>
